<template>
    <div>
        <Login v-if="login" v-on:estadoLogin="cambiarEstadoLogin" v-on:estadoRegister="cambiarEstadoRegister1"/>
        <Register v-if="register" v-on:estadoRegister="cambiarEstadoRegister2"/>

<template v-if="logged">
        
        <div :class="containerClass" @click="onWrapperClick"            log>
...
export default {
    data() {
        return {
            ...
            menu : [
                ...
                {
                    label: 'Eventos', icon: 'pi pi-fw pi-globe', to: '/eventos'
                },
                {
                    label: 'Proveedores', icon: 'pi pi-fw pi-globe', to: '/proveedor'
                },
                {label: 'Documentation', icon: 'pi pi-fw pi-question', command: () => {window.location = "#/documentation"}},
                {label: 'View Source', icon: 'pi pi-fw pi-search', command: () => {window.location = "https://github.com/primefaces/sigma-vue"}} */
                {label: 'Dashboard', icon: 'pi pi-fw pi-home', to: '/dashboard'},
                {label: 'Imagenes', icon: 'pi pi-fw pi-search', to: '/imagenes'}
            ],
            login: true,
            register: false,
            logged: false,
    ...
    methods: {
        ...
        cambiarEstadoProfile(valor){
            this.login = valor.login
            this.register = valor.register
            this.logged = valor.logged
            auth.deleteUserLogged()
        },
        async Inicializar(){
        ...
        cambiarEstadoLogin(valor){
            this.login = valor.login
            this.register = valor.register
            this.logged = valor.logged
            this.user = valor.user
            auth.setUserLogged(valor.user.email)
        },
        cambiarEstadoRegister1(valor){
            this.login = valor.login
            this.register = valor.register
            this.logged = false
        },
        cambiarEstadoRegister2(valor){
            this.login = valor.login
            this.register = valor.register
            this.logged = false
        },
